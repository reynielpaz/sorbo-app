...
### Tokens de tema (Tailwind CSS v4)

La paleta vive en la directiva `@theme` de `src/styles/globals.css`. Tailwind v4 genera
las utilidades `bg-*`, `text-*`, `border-*`, `from-*` y `to-*` directamente desde estos
tokens CSS.
...
Los gradientes, valores glassmorphism y sombras complejas permanecen en `:root`. Los
efectos reutilizados por componentes se exponen desde `@theme` para generar utilidades
nativas como `bg-sorbo-glass`, `border-sorbo-glass-border` y `shadow-sorbo-gold`.
...
| Texto secundario | Blanco 50% | `text-sorbo-cream/50` |
...
| Hover effects | Dorado sutil | `hover:border-sorbo-gold/40` |
...
Estado hover: scale-[1.02]
Sombra CTA: shadow-sorbo-gold
...
```
Base: bg-sorbo-warm, border border-sorbo-gold/10, rounded-xl
